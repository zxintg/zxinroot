...
public class CommonEditView extends LinearLayout {
    ...
    private void setParameter() {
        if (!isTopEdit)
            return;
        LayoutParams params = new LayoutParams(LayoutParams.MATCH_PARENT, SystemInfoUtil.getInstance(getContext()).dip2px(45) + SystemInfoUtil.getInstance(getContext()).getStatusBarHeight());
        editview.setPadding(0, editview.getPaddingTop() + SystemInfoUtil.getInstance(getContext()).getStatusBarHeight(), 0, 0);
    ...
    public void setLeftText() {
        ...
        if (left_image != -1)
            ImageUtil.getInstance(getContext()).setCompoundDrawable(leftText, 15, left_image, Gravity.LEFT, 0);
    ...
    public void setRightText() {
        ...
        if (right_image != -1)
            ImageUtil.getInstance(getContext()).setCompoundDrawable(rightText, 15, right_image, Gravity.RIGHT, 0);
