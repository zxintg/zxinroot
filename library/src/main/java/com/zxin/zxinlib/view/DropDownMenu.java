...
public class DropDownMenu extends RelativeLayout implements View.OnClickListener{
    ...
    public DropDownMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        tabLayout.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,SystemInfoUtil.getInstance(context).dpTpPx(menuTitleHeight)));
    ...
    public void setDropDownFragment(@NonNull List<String> tabTexts,@NonNull List<Fragment> popupFragment) {
        ...
        for (int i = 0; i < tabTexts.size(); i++) {
            ...
            int mWeight = (int)(SystemInfoUtil.getInstance(getContext()).getScreenWidth()*weight);
    ...
    private void addFragmentTab(@NonNull List<String> tabTexts, final Fragment fragment, final int index, boolean isShowLine, int weight) {
        ...
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(weight,SystemInfoUtil.getInstance(getContext()).dpTpPx(menuTitleHeight));
    ...
    private void mainOvlayStatus(boolean isOpen){
        if (mListener!=null){
            mListener.showOpenFlag(isOpen ? IntegerUtil.CONDITION_POPUP_OPEN : IntegerUtil.CONDITION_POPUP_ClOSE);
        }
    }


    public void setDropDownMenuListener(DropDownMenuListener mListener){
        this.mListener = mListener;
    }

    private DropDownMenuListener mListener;
    public interface DropDownMenuListener{
        void showOpenFlag(int openFlag);
    }
