...
public class PagerSlidingVerTabStrip extends NestedScrollView {
    ...
    public PagerSlidingVerTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        int ScreenHeight = SystemInfoUtil.getInstance(getContext()).getScreenHeight();
        setFillViewport(true);
        setWillNotDraw(false);

        scrollOffset = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, scrollOffset, SystemInfoUtil.getInstance(getContext()).getDisplayMetrics());
        indicatorHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, indicatorHeight, SystemInfoUtil.getInstance(getContext()).getDisplayMetrics());
        underlineWeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, underlineWeight, SystemInfoUtil.getInstance(getContext()).getDisplayMetrics());
        dividerPadding = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dividerPadding, SystemInfoUtil.getInstance(getContext()).getDisplayMetrics());
        dividerWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dividerWidth, SystemInfoUtil.getInstance(getContext()).getDisplayMetrics());
        tabTextSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, tabTextSize, SystemInfoUtil.getInstance(getContext()).getDisplayMetrics());
        selectedTabTextSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, selectedTabTextSize, SystemInfoUtil.getInstance(getContext()).getDisplayMetrics());
        ...
        pstsVerTextBgColor = a.getColor(R.styleable.PagerSlidingVerTabStrip_pstsVerTextBgColor, UiUtils.getInstance(getContext()).getColor(R.color.color_ffffff));
        pstsVerTextCheckedBgColor = a.getColor(R.styleable.PagerSlidingVerTabStrip_pstsVerTextCheckedBgColor, UiUtils.getInstance(getContext()).getColor(R.color.color_ffffff));
    ...
    private void addTextTab(final int position, String title, int tabCount) {
        ...
        int padd = SystemInfoUtil.getInstance(getContext()).dip2px(10);
        ...
        if (pstsVerHasImage && pager.getAdapter() instanceof ViewPageFragmentAdapter) {
            ...
            ImageUtil.getInstance(getContext()).setCompoundDrawable(tab, pstsVerImageSize, titleBean.labImage, Gravity.TOP, 5);
        }
        tab.setText(title);
