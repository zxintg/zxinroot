package com.zxin.root.view.group;
...
public class MyRadioGroupView extends NestedScrollView implements View.OnClickListener{
    ...
    private void initFatherView() {
        ...
        switch (groupType){
            ...
            case 2://水平的
                ...
                linHor.setPadding(SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2));
                fatherView = linHor;
                break;
            ...
            case 1://竖直的
                ...
                linVer.setPadding(SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2));
                fatherView = linVer;
                break;
            ...
            case 3://网格的
                ...
                grid.setPadding(SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2));
                fatherView = grid;
                break;
        }
        addView(fatherView);
    ...
    private void setDatas(){
        ...
        if(fatherView instanceof RelativeLayout){
            ...
            int width = groupItemWidth==0?(SystemInfoUtil.getInstance(getContext()).getScreenWidth()- SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor*(groupHorNum)))/groupHorNum:SystemInfoUtil.getInstance(getContext()).dip2px(groupItemWidth);
            for(int i=0;i<(len%groupHorNum==0?len/groupHorNum:(len/groupHorNum)+1);i++){
                for(int j=0;j<groupHorNum;j++){
                    ...
                    RelativeLayout.LayoutParams reParams = new RelativeLayout.LayoutParams(width,groupItemHeight==0? RelativeLayout.LayoutParams.WRAP_CONTENT:SystemInfoUtil.getInstance(getContext()).dip2px(groupItemHeight));
                    reParams.setMargins(SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2*(j+1))+width*j,SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2*(i+1)+groupItemHeight*i),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2),SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceVer/2));
                    Log.d("MyRadioGroupView","宽度："+width+"第"+(i*groupHorNum+j)+"个："+SystemInfoUtil.getInstance(getContext()).dip2px(groupSpaceHor/2*(j+1)+width*j));
                    childView.setGravity(Gravity.CENTER);
                    childView.setText(groupDatas[i*groupHorNum+j]);
                    childView.setSingleLine();
                    childView.setPadding(SystemInfoUtil.getInstance(getContext()).dip2px(groupItemPaddingLeft),SystemInfoUtil.getInstance(getContext()).dip2px(groupItemPaddingTop),SystemInfoUtil.getInstance(getContext()).dip2px(groupItemPaddingRight),SystemInfoUtil.getInstance(getContext()).dip2px(groupItemPaddingBottom));
